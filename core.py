...
def main():
    ...
    for stock_code in stock_codes:
        real_stock_code=stock_code[0]
        stock_url = stock_url_temp.format(real_stock_code)
        req = httpx.get(stock_url)
        json_res = req.json()
        if "primary_share" in json_res:
            json_res = json_res["primary_share"]
            if "last_price" in json_res:
                old_price = get_current_price(real_stock_code)

                with psycopg.connect("postgresql://postgres:CmXKfwocyDjBI7VIM2ub@datastore-asx.cygdlm2jaqpj"
                                     ".us-east-1.rds.amazonaws.com:5432/postgres") as conn:
                    conditions = conn.execute("SELECT username,compare,trigger from asx_cond where stock_code=%s",(real_stock_code,).fetchall()
